    ...
    for epoch in range(args.epochs):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            if (idx + 1) % args.log_interval == 0:
                ...
                print(
                    ...
                    f"training loss {train_loss:4.4} || train f1 loss {train_f1_loss:4.4} || training accuracy {train_acc:4.2%} || lr {current_lr}"
                )
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                f1_loss_item = criterion2(outs, labels).item()
                acc_item = (labels == preds).sum().item()

                val_loss_items.append(loss_item)
                val_f1_loss_items.append(f1_loss_item)
            ...
            val_f1_loss = np.sum(val_f1_loss_items) / len(val_loader)
            val_acc = np.sum(val_acc_items) / len(val_set)
            best_val_loss = min(best_val_loss, val_loss)

            if val_f1_loss < best_val_f1_loss:   
                print(f"New best model for val f1 loss : {val_f1_loss:4.4}! saving the best model..")
                torch.save(model.module.state_dict(), f"{save_dir}/best.pth")
                best_val_f1_loss = val_f1_loss
            torch.save(model.module.state_dict(), f"{save_dir}/last.pth")
          
            print(f"[Val] acc : {val_acc:4.2%} || f1: {val_f1_loss:4.2} || loss: {val_loss:4.2} || best f1 : {best_val_f1_loss:4.4} || best loss: {best_val_loss:4.4}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--criterion2', type=str, default='f1', help='criterion type (default: f1)')
